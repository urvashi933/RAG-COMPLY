...
def rag_answer(query:str, sector:str, user_id:int=None) -> dict :
    ...
    # 1 Log every user question (for history & analytics)
    ...
    # 3 If not found, try other sectors
    ...
    # 4 Still nothing ➜ handle definition / unanswered
	if not retrieved_chunks:
    ...
    # 5 Build strict RAG prompt using retrieved chunks
	prompt = build_prompt(retrieved_chunks, query)

    # 6 Generate answer using Gemini (2.5 Flash-Lite)
	answer = generate_answer(prompt)

    # 7 Collect unique document sources for citation
	sources = list(
